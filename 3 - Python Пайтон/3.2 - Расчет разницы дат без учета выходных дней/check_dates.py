...
class DateChecker():
    ...
    def get_dates_difference(self,
        ...
        :param
        regime
              - при значении 'all_days' считается разница между датами с учетом всех дней;
              - при значении 'work_days' считается разница между датами с учетом только будних дней
        ...
        if regime == 'work_days':
            ...
            diff_dict['days'] = work_days_num
            difference_days = work_days_num
        # проверка на валидность (вторая дата должна быть позже первой!!!)
